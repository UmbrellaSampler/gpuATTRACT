...
c     Local variables
      integer n,nn,i,jl
      real*8 ub,sqrt_et,et,et2,xd,yd,zd,xf,yf,zf,k

      erest = 0.0d0
      ub = 0.0d0
      call cartstate_f_disre(cartstatehandle,nlig,ptr_pivot)
      call ministate_f_disre(ministatehandle,gravity,rstk,ub)
...
C$$$            et=(xd**2+yd**2+zd**2)
C$$$            instead we use a bounded constraint, lb default 0, ub requires input
      sqrt_et=sqrt(xd**2+yd**2+zd**2)
      if (sqrt_et>ub) then
         et=(sqrt_et-ub)**2
